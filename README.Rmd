...
- `claim`: my workflow of cleaning data:
  - keep original data intact and claim new varible to a predefined data (default is '.kollection'), e.g.,
  ```r
  original_data |> mutate(new_var = ...cleaning/recoding...) |> claim(new_var) # return the invisible mutated data
  ```
  then `.kollection` now have `new_var` collected. One can claim multiple as
  `claim(a,b,c, .name = "my_cleaned_data")` to store to your own named data
  instead of `.kollection`.
